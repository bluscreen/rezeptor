...
import de.dhbw.rezeptor.HelpManager;
import de.dhbw.rezeptor.RezeptorUI;
import de.dhbw.rezeptor.data.Generator;
/**
 * die klasse dient nur als vorlage... sie wird später nicht mehr da sein!!
 * @author dhammacher
 *
 */
public class View_Reports extends HorizontalLayout implements View {
...
//                                .updateReportsButtonBadge(draftCount + "");
    ...
    private HorizontalLayout createEditorInstance(int which,
        ...
        if (!helpShown) {
            helpShown = true;
            HelpManager helpManager = ((RezeptorUI) getUI()).getHelpManager();
...
//        ((RezeptorUI) UI.getCurrent()).updateReportsButtonBadge(draftCount
//                + "");
...
//                t.setContainerDataSource(((RezeptorUI) getUI()).dataProvider
//                        .getRevenueByTitle());
                t.setWidth("100%");
                t.setPageLength(0);
